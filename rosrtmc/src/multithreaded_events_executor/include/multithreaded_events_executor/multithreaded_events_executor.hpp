#ifndef ROSRTMC__MULTITHREADED_EVENTS_EXECUTOR__EVENTS_EXECUTOR_HPP_
#define ROSRTMC__MULTITHREADED_EVENTS_EXECUTOR__EVENTS_EXECUTOR_HPP_
...
#include "multithreaded_events_executor/multithreaded_events_queue.hpp"
 ...
 * rclcpp::experimental::executors::MultiThreadedEventsExecutor executor;
 * executor.add_node(node);
 * executor.spin();
 * executor.remove_node(node);
 */
class MultiThreadedEventsExecutor : public rclcpp::Executor
{
  friend class EventsExecutorEntitiesCollector;

  using ExecutorEvent = rclcpp::experimental::executors::ExecutorEvent;
  using ExecutorEventType = rclcpp::experimental::executors::ExecutorEventType;
...
public:
  ...
  /**
   * \param[in] num_threads Number of threads to use for execution.
   ...
   */
  RCLCPP_PUBLIC
  explicit MultiThreadedEventsExecutor(
    const size_t num_threads = 1,
    rclcpp::experimental::executors::EventsQueue::SharedPtr events_queue = std::make_shared<MultiThreadedEventsQueue>(),
  ...
  RCLCPP_PUBLIC
  void
  spin_some(std::chrono::nanoseconds) override 
  {
    throw std::runtime_error("Cannot call spin_some() on MultiThreadedEventsExecutor. Use spin() instead.");
  }
  ...
  {
    throw std::runtime_error("Cannot call spin_all() on MultiThreadedEventsExecutor. Use spin() instead.");
  }
  ...
  /**
   * \sa rclcpp::MultiThreadedEventsExecutor::add_node
   */
  RCLCPP_PUBLIC
  void
...
protected:
  RCLCPP_PUBLIC
  void
  spin_once_impl(std::chrono::nanoseconds) override
  {
    throw std::runtime_error("Cannot call spin_once_impl() on MultiThreadedEventsExecutor. Use spin() instead.");
  }
...
private:
  ...
  /// Number of threads to use for spinning
  size_t num_threads_;

  /// Pool of threads used for spinning
  std::vector<std::thread> spinning_threads_;
public:
  /// Number of threads currently idle
  std::atomic<size_t> num_idle_;
};
...
#endif  // ROSRTMC__MULTITHREADED_EVENTS_EXECUTOR__EVENTS_EXECUTOR_HPP_
