...
#ifndef ROSRTMC__MULTITHREADED_EVENTS_EXECUTOR__SIMPLE_EVENTS_QUEUE_HPP_
#define ROSRTMC__MULTITHREADED_EVENTS_EXECUTOR__SIMPLE_EVENTS_QUEUE_HPP_
...
namespace executors
{
  using EventsQueue = rclcpp::experimental::executors::EventsQueue;
 ...
 */
class MultiThreadedEventsQueue : public EventsQueue
{
public:
  RCLCPP_PUBLIC
  ~MultiThreadedEventsQueue() override = default;
    ...
    std::chrono::nanoseconds timeout = std::chrono::nanoseconds::max()) override
  {
    ...
    if (timeout != std::chrono::nanoseconds::max()) {
      events_queue_cv_.wait_for(lock, timeout, [this]() {return !event_queue_.empty() || shutdown_;});
    } else {
      events_queue_cv_.wait(lock, [this]() {return !event_queue_.empty() || shutdown_;});
    }
...
#endif  // ROSRTMC__MULTITHREADED_EVENTS_EXECUTOR__SIMPLE_EVENTS_QUEUE_HPP_
