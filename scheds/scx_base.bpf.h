#ifndef __SCX_BASE_BPF_H
#define __SCX_BASE_BPF_H
...
union policy_task_ctx {
  u64 __align;
  u8 data[BASE_POLICY_TASK_CTX_SIZE];
};
...
struct base_task_ctx {
  u64 tid;
};
typedef struct base_task_ctx __arena base_task_ctx_t;
_Static_assert(sizeof(struct base_task_ctx) <= BASE_POLICY_TASK_CTX_SIZE, "base_task_ctx larger than BASE_POLICY_TASK_CTX_SIZE");
_Static_assert(_Alignof(struct base_task_ctx) <= _Alignof(union policy_task_ctx), "base_task_ctx requires greater alignment");

static __always_inline base_task_ctx_t *get_base_task_ctx(task_ctx_t *tctx) {
  return likely(tctx) ? (base_task_ctx_t *)tctx->ptctx.data : (base_task_ctx_t *)0;
}
...
__hidden struct bpf_res_spin_lock base_arena_task_ctx_slab_lock SEC(".data.base_arena_task_ctx_slab_lock");
...
static __always_inline void root_init() {
  ...
  if (nr_cids > BASE_MAX_CPUS)
    nr_cids = BASE_MAX_CPUS;
  ...
  topo->nr_nodes = 0;
  u32 cid;
  u32 no_topo_core = BASE_MAX_CPUS;
  u32 no_topo_shard = BASE_MAX_CPUS;
  u32 no_topo_llc = BASE_MAX_CPUS;
  u32 no_topo_node = BASE_MAX_CPUS;
  bpf_for(cid, 0, nr_cids) {
    ...
    struct core_topo_data *core_td = &topo->cores[t.core_idx & (BASE_MAX_CPUS-1)];
    struct shard_topo_data *shard_td = &topo->shards[t.shard_idx & (BASE_MAX_CPUS-1)];
    struct llc_topo_data *llc_td = &topo->llcs[t.llc_idx & (BASE_MAX_CPUS-1)];
    struct node_topo_data *node_td = &topo->nodes[t.node_idx & (BASE_MAX_CPUS-1)];
  ...
  bpf_for(i, 0, nr_shards) {
    struct shard_topo_data *shard_td = &topo->shards[i & (BASE_MAX_CPUS-1)];
    struct llc_topo_data *llc_td = &topo->llcs[shard_td->llc_idx & (BASE_MAX_CPUS-1)];
    struct node_topo_data *node_td = &topo->nodes[shard_td->node_idx & (BASE_MAX_CPUS-1)];
    ...
    bpf_for(j, 0, end) {
      shard_td->shard_dist_order[(1 + j) & (BASE_MAX_CPUS-1)] = llc_td->base_shard + j + (j < off ? (u32)0 : (u32)1);
    }
    ...
    bpf_for(j, 0, end) {
      shard_td->shard_dist_order[(llc_td->nr_shards + j) & (BASE_MAX_CPUS-1)] = node_td->base_shard + j + (j < off ? (u32)0 : llc_td->nr_shards);
    }

    // other nodes: [node->nr_shards, topo->nr_shards - node->nr_shards]
    off = node_td->base_shard;
    end = nr_shards - node_td->nr_shards;
    if (unlikely(end >= BASE_MAX_CPUS-off)) return; // for verifier, should not happen
    bpf_for(j, 0, end) {
      shard_td->shard_dist_order[(node_td->nr_shards + j) & (BASE_MAX_CPUS-1)] = j + (j < off ? (u32)0 : node_td->nr_shards);
    }
...
// init base_arena
static __always_inline s32 base_init(struct base_arena __arena *a) {
  if (a->cgroup_id == 0) {
    root_init();
  }
  ...
  u32 nr_pages = (a->max_tasks * TASK_CTX_STRIDE + PAGE_SIZE - 1) / PAGE_SIZE;
	...
	bpf_for(i, 0, a->max_tasks) {
		task_ctx_t *curr = (task_ctx_t *)(slab + i * TASK_CTX_STRIDE);
		task_ctx_t *next = (i + 1 < a->max_tasks) ?
...
static __always_inline struct sub_sched_ctx __arena *sub_lookup(struct base_arena __arena *a, u64 cgroup_id) {
...
static __always_inline task_ctx_t *base_init_task(struct base_arena __arena *a, struct task_struct *p, struct scx_init_task_args *args) {
  // allocate new task_ctx_t (from qmap)
  /* pop a slab entry off the free list */
	if (unlikely(bpf_res_spin_lock(&base_arena_task_ctx_slab_lock))) {
	...
	bpf_res_spin_unlock(&base_arena_task_ctx_slab_lock);
  ...
  // init base_task_ctx fields
  base_task_ctx_t *base_tctx = get_base_task_ctx(tctx);
  base_tctx->tid = p->scx.tid;

  return tctx;
}

// returns true if successful
static __always_inline bool base_exit_task(struct base_arena __arena *a, struct task_struct *p) {
  ...
  if (unlikely(!ptr || !ptr->tctx)) return false; // for verifier, should not happen
	...
	if (bpf_res_spin_lock(&base_arena_task_ctx_slab_lock)) {
    scx_bpf_error("failed to acquire task_ctx slab lock");
    return false;
  }
	tctx->next_free = a->task_free_head;
...
static __always_inline task_ctx_t *base_set_cmask(struct task_struct *p, const struct scx_cmask *cmask_in) {
...
static __always_inline void base_update_idle(struct base_arena __arena *a, s32 cid, bool idle) {
  if (idle)
